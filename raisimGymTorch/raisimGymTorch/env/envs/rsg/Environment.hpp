...
class ENVIRONMENT : public RaisimGymEnv {
 ...
 public:
  ...
  explicit ENVIRONMENT(const std::string& resourceDir, const Yaml::Node& cfg, bool visualizable) :
    ...
    //anymal_=world_->addArticulatedSystem(resourceDir_+"/bolt_description/robots/bolt.urdf");
    anymal_ = world_->addArticulatedSystem(resourceDir_+"/anymal/urdf/anymal.urdf");
    anymal_->setName("dog");
    anymal_->setControlMode(raisim::ControlMode::PD_PLUS_FEEDFORWARD_TORQUE);
    world_->addGround(0,"steel");
    world_->setDefaultMaterial(0.1,0,0);
    world_->setMaterialPairProp("steel", "steel", 1, 1.0, 0.0);
...
//    rewards_.record("forwardVel", std::mi);
    ...
    //rewards_.record("forwardVel", (-abs(bodyLinearVel_[0])-abs(bodyLinearVel_[1])+abs(bodyLinearVel_[2])), false);
  ...
  bool isTerminalState(float& terminalReward) final {
    ...
    for(auto& contact: anymal_->getContacts())
      if(footIndices_.find(contact.getlocalBodyIndex()) == footIndices_.end())
        {std::cout << "stable down" << contact.getlocalBodyIndex() << std::endl;
...
//   }
 ...
 private:
  ...
  raisim::ArticulatedSystem* anymal_, *anymal_1,*leg;
